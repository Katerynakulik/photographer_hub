...
def home(request):
    photographer = UserProfile.objects.filter(
        is_photographer=True).select_related('user').first()
    featured_products = PhotoProduct.objects.filter(
...
def contact(request):
    ...
    if form.is_valid():
        ...
        messages.error(
            request, "There was an error with your submission. Please check the form.")
